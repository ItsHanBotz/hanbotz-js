...
module.exports = class Function {
   ...
   switch (type) {
      ...
      case 'smallcaps':
         ...
         const smallcapsText = text.toLowerCase().split('').map(char => smallcapsMap[char] || char).join('');
   ...
   simpFormat = (str) => {
   try {
      let dotCount = str.split('.').length - 1;
      let getF = parseInt(str.split('.')[1]);
      let output;

      if (dotCount === 1) {
         output = str.replace('.', '') + 'Rb';
      } else if (dotCount === 2) {
         output = str.replace('.', '') + 'Jt';
      } else if (dotCount === 3) {
